...
> Week 20 · Phase 3 · Machine Learning · time box: **12-14 h**
...
Fraud losses were £1.8M last year. The rules engine flags 3% of transactions; the ops team can
review 0.4% of them. So 87% of what the system flags is never looked at, and the fraud that gets
through is the fraud nobody had a rule for.

Build the model. But the deliverable is not the model — it is the **threshold**, chosen against
an explicit cost matrix, with the expected annual saving written down and defensible to someone
who will ask where the numbers came from.

## What "done" looks like

- A PR curve, not an ROC curve, because the positive class is 0.17% of the data.
- A threshold chosen by expected cost, with the cost matrix stated.
- An expected annual saving with its assumptions listed.
...
Repo plus the cost analysis. Pairs with the Week-19 harness — run this pipeline through it
and put the report in the README.
