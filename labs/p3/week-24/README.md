...
> Week 24 · Phase 3 · Machine Learning · time box: **12-15 h**
...
The Week-23 Lambda works until it does not: a load spike during a customer demo put a
nine-second cold start in front of the CEO, and the fix suggested in the retro was "provisioned
concurrency", which triples the bill.

Move it to Kubernetes, make it scale on a signal that reflects actual load, and write the runbook
the on-call engineer will read at 3 a.m. — which means it has to say what to do, not what the
system is.

## What "done" looks like

- Scales up under load and back down after, demonstrated with a graph.
- A rolling update under sustained load with zero failed requests, proven.
- A runbook with the three most likely failures and the exact commands.
...
Repo plus `RUNBOOK.md` plus the scaling report. The runbook is the artifact — write it for
someone who has been woken up.
