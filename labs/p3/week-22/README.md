...
> Week 22 · Phase 3 · Machine Learning · time box: **12-14 h**
...
A pathology lab wants a pre-screen for their blood-cell slides. Two hard constraints from
them: they will not deploy anything below **90% test accuracy**, and their hardware is a CPU box
in a room with no GPU and no plans for one.

Train it, hit the bar, and give them the latency and size table for the model you would actually
ship — not the one that scored best.

## What "done" looks like

- >= 90% accuracy on a test split that was never touched during development.
- Per-class metrics, because the rare class is the one they care about.
- An error analysis of the twenty worst misclassifications with a written pattern.
...
Repo plus a deployed demo — the serving API is next week's lab, so build this one so it can
be picked up as an artifact.
