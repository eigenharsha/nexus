...
> Week 27 · Phase 4 · Generative AI · time box: **14-16 h**
...
Legal has 1,000 PDFs of contracts and answers questions about them with Ctrl-F, one file at a
time. They have asked for "search, but good".

Build the retrieval system — and, more importantly, build the 100-query evaluation set first,
because otherwise you will spend two weeks tuning chunk sizes with no way to tell whether you
made it better or worse. The eval set is the deliverable that makes the rest of it credible.

## What "done" looks like

- A 100-query eval set with judged relevant documents.
- Recall@k and NDCG reported for every configuration you tried, not just the one you shipped.
- HNSW parameters chosen from measurements, not from the defaults.
...
Repo plus `RETRIEVAL.md` with the ablation table and the HNSW parameter study.
