...
> Week 30 · Phase 4 · Generative AI · time box: **14-16 h**
...
Support wants an agent that can look up an order, check inventory, and run a diagnostic
command on the box. The third capability is why security has blocked this twice, and they were
right to.

Build the agent from scratch — no framework, so you own the loop — and the MCP server that
exposes those capabilities behind a trust boundary you can defend in a security review. The
`SECURITY.md` is not documentation here; it is the thing being reviewed.

## What "done" looks like

- An agent loop with a bounded token budget and structured stop conditions.
- An MCP server whose sandbox you have personally tried to escape.
- A 30-task benchmark with a success rate.
...
Repo plus `SECURITY.md` documenting the MCP server's trust boundary. A rare and highly
credible artifact — very few candidates have written one.
