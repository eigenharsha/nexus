...
> Week 31 · Phase 4 · Generative AI · time box: **14-16 h**
...
A prompt change last sprint silently broke 12% of extraction cases. Nobody noticed for nine
days, and we found it because a customer sent a screenshot.

Build the eval suite that fails the build instead — running in CI on every prompt or code change,
commenting the diff on the PR. Then the guardrail layer, with measured false-positive rates,
because a guardrail that blocks 4% of legitimate traffic gets switched off in a week and then you
have neither.

## What "done" looks like

- CI fails the build on a quality regression, with a PR comment showing what changed.
- An LLM judge validated against human labels with an agreement number.
- Guardrail FP/TP rates measured, not assumed.
...
Repo plus `EVALS.md` plus `RED-TEAM.md`. This is the artifact that most distinguishes a
serious AI engineer from a demo builder.
