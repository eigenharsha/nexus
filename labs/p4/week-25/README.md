...
> Week 25 · Phase 4 · Generative AI · time box: **14-16 h**
...
Every architecture decision for the next eight weeks comes from the shape of the tensors
inside a Transformer: why your context window costs what it costs, why the KV cache is the thing
that actually limits your batch size, why the fine-tune OOMs at batch 4 and not batch 8.

Build one. Tokenizer, attention, blocks, KV-cached generation — each component unit-tested
against its PyTorch equivalent, so that when you read a paper in Week 27 you are reading about
code you have written.

## What "done" looks like

- Your BPE tokenizer round-trips any UTF-8 string exactly.
- Every component matches its `torch.nn` equivalent to within tolerance on the same weights.
- KV-cached generation produces token-identical output to the uncached path.
...
Repo plus an illustrated "how a Transformer works" explainer built from your own diagrams.
The highest-signal portfolio piece in the course for AI roles.
