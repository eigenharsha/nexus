...
> Week 14 · Phase 2 · Systems & Data · time box: **12-15 h**
...
You have an on-site loop in three weeks with four algorithm rounds. Separately, ops wants a
routing CLI for the field-service fleet and has been asking since March.

Do both with the same fifteen hours: fifteen medium problems with a written complexity analysis
for each, and a Dijkstra route planner on real map data. The problems are the interview; the
planner is the thing you can actually show someone.

## What "done" looks like

- Fifteen problems, each with the approach, the complexity, and the first idea that did not work.
- A route planner that answers a query on a real city graph in under a second.
...
A "problem-solving journal" repo: for each problem, the approach, the failed first idea, and
the final complexity. This is a stronger interview artifact than the solutions themselves.
