...
> Week 15 · Phase 2 · Systems & Data · time box: **12-14 h**
...
Finance ops pulls a partner's transaction API by hand every Monday, pastes it into a sheet,
dedupes by eye, and reports the number by Wednesday. Twice this year the number was wrong: once
because the partner added a column, once because a page was pulled twice.

Automate it end to end. Incremental, deduplicated, schema-evolving, restartable — and with a run
report that tells you when to distrust the number, because a pipeline that silently produces a
wrong number is worse than a person with a spreadsheet.

## What "done" looks like

- A re-run does not duplicate a single row, proven by a row count and a hash.
- A new column in the source does not break the run.
- A crash halfway through resumes without double-counting.
...
Repo plus `PIPELINE.md` with a dataflow diagram and the run report from a real execution.
