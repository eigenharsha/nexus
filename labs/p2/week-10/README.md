...
> Week 10 · Phase 2 · Systems & Data · time box: **10-12 h**
...
The internal deploy-notification bot runs on a third-party chat SDK we no longer trust and
cannot debug. When messages arrive interleaved and unreadable, the vendor tells us it is our
network. We have no way to prove otherwise.

Build the server ourselves on raw sockets, so the framing is ours and so that the next time
somebody says "it's the network" we can put a packet capture on the table.

## What "done" looks like

- A written protocol spec precise enough that someone else could implement a client from it.
- A Wireshark/tshark capture showing your framing on the wire.
- Correct behaviour when a client vanishes without closing the socket.
...
Repo with `PROTOCOL.md`, the benchmark report, and the packet captures.
