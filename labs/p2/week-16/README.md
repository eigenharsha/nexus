...
> Week 16 · Phase 2 · Systems & Data · time box: **10-12 h**
...
Every new dataset that lands here costs the same three days of "let me just look at it"
before anyone will commit to a model. Two of those days are the same work every time.

Build the tool that does them: point it at a CSV or Parquet and get an HTML report with schema,
missingness, distributions, outliers, correlations, target relationships — and the leakage
warnings, which is the part nobody remembers to check and the reason the churn model last year
had a 0.99 AUC in dev and 0.61 in production.

## What "done" looks like

- One command, any tabular file, an HTML report you would send to a stakeholder.
- Leakage warnings that would have caught last year's incident.
- Tested on three structurally different datasets.
...
A publishable tool plus the report artifacts. Pairs with the Week-15 pipeline into one
"raw source to analysis" system you can demo in a single command.
