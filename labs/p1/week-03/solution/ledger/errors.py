"""Every error this library raises. One root, so a caller can catch `LedgerError`
and know it caught something from us and not from the standard library."""
...
class UnknownAccount(LedgerError):
    """A posting referenced an account that was never registered."""
...
class DuplicateTransaction(LedgerError):
    """A transaction with this id has already been posted."""
