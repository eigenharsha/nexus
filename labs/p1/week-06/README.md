...
> Week 6 · Phase 1 · Foundations · time box: **10-12 h**
...
The careers page wants a "tailor my resume to this job" button, and marketing has already
promised it for the end of the month.

Build the service behind it: upload a resume, POST a job description, get structured Markdown
back. **No LLM yet.** Rules only. We add the model in Week 26, and when we do we want the API,
the validation, the limits, the auth and the tests to already be right — so that when the model
starts behaving badly we know it is the model.

## What "done" looks like

- OpenAPI docs a frontend engineer can build against without asking you anything.
- Uploads that are too big, the wrong type, or malicious are rejected before they touch disk.
- 25+ tests, including the unhappy paths.
- A load-test report with a p95 number.
